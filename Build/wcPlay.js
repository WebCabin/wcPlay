      ...
      reader.onload = function(e) {
        if (self._engine) {
          if (importing) {
            ...
            if (self._engine.importComposite(e.target.result, file.name)) {
              self.__setupPalette();
              self.$typeButton[3].click();
            }
          } else {
            if (self._engine.restore(e.target.result)) {
              self._parent = self._engine;
